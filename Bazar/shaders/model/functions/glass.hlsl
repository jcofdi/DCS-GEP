...
void calcGlassColors(VS_OUTPUT input, uniform int Flags, out float4 colorMul, out float4 colorAdd)
{
	...
	static const float d0 = 1.0; // medium thickness at normal incidence
	static const float n1 = 1.000292; // air
	static const float n2 = 1.49; // plexiglass (PMMA)
	// static const float n2 = 1.334; // water
	static const float F0 = (n1 - n2) * (n1 - n2) / ((n1 + n2) * (n1 + n2)); // 0.0387 for PMMA
	...
	// Two-surface slab Fresnel: Schlick approximation with F0 from IOR.
	// Entry surface uses external view angle. Exit surface uses the
	// refracted internal angle from Snell's law (getCosGamma), which is
	// always shallower than the external angle for n2 > n1.
	float NoV = dot(mp.normal, mp.toCamera);
	float F_entry = F0 + (1.0 - F0) * pow(saturate(1.0 - NoV), 5.0);
	float cosGamma = getCosGamma(NoV, n1, n2);
	float F_exit = F0 + (1.0 - F0) * pow(saturate(1.0 - cosGamma), 5.0);
	float Ft = 1.0 - F_entry;

	#if CALC_GLASS_TRANSMITTANCE
		float d = d0 * getRefractedRayLength(NoV, n1, n2);
		float3 scatteringCoef = getScatteringCoef(T0, d0);
		colorMul.rgb *= getTransmittance(scatteringCoef, d) * (Ft * (1.0 - F_exit));
		colorAdd.rgb += getTransmittance(scatteringCoef, d * 4.0) * AmbientAverage * (F_entry * inscatterFactor) * transmittance;
	#else
		colorMul.rgb *= T0 * (Ft * (1.0 - F_exit));
		colorAdd.rgb += T0 * AmbientAverage * (F_entry * inscatterFactor) * transmittance;
	#endif
