	...
	switch(shadingModel){
		case SHADING_STANDARD:
			return BuildGBuffer(input.Position.xy,
						...
						sv_sampleIndex,
#endif
						mp.diffuse, normal, mp.aorms.xyzw, mp.emissive, motion, 0.5, geoNormal);
		case SHADING_EMISSIVE:
			return BuildGBuffer(input.Position.xy,
#if USE_SV_SAMPLEINDEX
						sv_sampleIndex,
#endif
						mp.diffuse, normal, mp.aorms.xyzw, mp.emissive * mp.diffuse.a, motion, 0.5, geoNormal);
	}
	return BuildGBuffer(input.Position.xy,
#if USE_SV_SAMPLEINDEX
		sv_sampleIndex,
#endif
		float4(1,0,0,1), normal, mp.aorms.xyzw, float4(1,0,0,1), float2(0,0), 0.5, geoNormal);
}
