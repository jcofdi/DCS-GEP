...
HS_OUTPUT dsCluster( HS_CONST_OUTPUT input, float2 UV : SV_DomainLocation, const OutputPatch<HS_OUTPUT, 1> patch )
{
	...
	const float verticalGradientFactor = 0.45;//JC - was 0.15 -- previous comment: имитация эмбиентного затения к низу кластера в тени
...
// was float NoL = max(0, dot((color.xyz*2.0 - 254.0/255.0), -i.sunDirM.xyz)*0.5+0.5); -- reduced to allow less sun passthrough of cloud particles
	float NoL = max(0, dot((color.xyz*2.0 - 254.0/255.0), -i.sunDirM.xyz)*0.15+0.25);
