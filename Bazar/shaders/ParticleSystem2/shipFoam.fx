/*
	...
	(Foam on water behind ship + spray from its bow)
*/
...
VS_OUTPUT VS(float4 params		: TEXCOORD0, // UV, random[0..1], age
			 float3 startPos	: TEXCOORD1, // start position in world space
			 float4 startVelIn: TEXCOORD2) // start velocity in world space
{
...
// physically accurate white/near-white foam.
// Real ocean foam is aerated water — achromatic white with albedo ~0.4-0.7.
// Original was float3(0.025..0.0625, 0.1, 0.1) — very dark and green-tinted,
// making the wake trail appear as a dark murky streak instead of white turbulence.
//
// [MOD] Issue S5: Added shadow sampling to the stern trail foam.
// The original had no shadow term at all — the wake was equally bright in cloud
// shadow or full sun. Now we sample cloud shadows at the particle position and
// apply them to the sun contribution.
...
static const float bowAngle = 90*PI/180;
...
// [MOD] Issue S4: Corrected bow spray sun contribution.
// Original: (0.1 + shadow * 0.4) gave 10% sun even in full shadow and only 50%
// in full light — missing half the sun energy. Now uses proper shadow-gated
// hemisphere-averaged NoL (~0.5) plus a forward-scatter boost factor for the
...
float4 PS_bow(PS_INPUT_BOW i, uniform bool FLIR) : SV_TARGET0
{
	...
	if (FLIR) {
		...
		// [MOD] Albedo modulated by foam density — thick spray is brighter.
		float albedo = 0.4 + 0.2 * clrFoam; // [MOD] was const 0.6

		// [MOD] Proper shadow-gated sun with hemisphere-averaged NoL.
		// Spray particles are billboard cards without normals, so we use
		// average hemispherical NoL ≈ 0.5 plus a slight forward-scatter
		// boost. The 0.02 ambient floor (from skylight on spray even in shadow)
		// replaces the non-physical 0.1 constant.
		float sunNoL = 0.55; // hemisphere-avg + slight forward scatter boost
		float3 sunColor = getPrecomputedSunColor(0) * ((0.02 + i.shadow * sunNoL) / PI); // [MOD] was (0.1 + shadow * 0.4)
...
// [MOD] Issue S7: Changed foam alpha from additive to MAX, matching bowwave.fx fix.
BlendState foamOnly {
	BlendEnable[0] = TRUE;
	SrcBlend = ZERO;
	DestBlend = ONE;
