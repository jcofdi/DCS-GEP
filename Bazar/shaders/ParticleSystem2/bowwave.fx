...
// [MOD] Issue S1: Reduced Kelvin wake normal Y-bias from +2 to +1.
// The original +2 heavily flattened the wake normals, making the Kelvin V-pattern
// and transverse wave crests nearly invisible — they couldn't catch specular
// highlights at oblique sun angles. Real Kelvin wakes have prominent wave slopes
// (~5-15 degree deflections). A bias of +1 doubles the effective normal perturbation
// while still keeping the average surface pointing upward.
float4 KelvinWakePS(VS_OUTPUT i) : SV_TARGET0 {
	...
	normalMap.y += 1.0; // [MOD] was += 2; stronger wake wave normals
...
// [MOD] Issue S2: Fixed tangent basis for normal reconstruction.
// Original used float3(0, -1, ...) which inverted the tangent Y, subtly
// affecting the normalized horizontal components. Now uses standard +Y up.
// The *10 slope scale is retained as the baseline but documented for future
// per-ship tuning via a uniform.
float4 sampleShipWake(float3 uv) {
	...
	// [MOD] Corrected second tangent vector: (0, +1, ...) instead of (0, -1, ...)
	// This ensures the cross product produces a normal consistent with +Y = up.
...
// [MOD] Issue S7: Changed foam alpha accumulation from additive to MAX.
// Additive ONE+ONE allowed foam alpha to exceed 1.0 in overlap areas (hull
// foam + Kelvin foam + wake foam near the bow). MAX gives physically
// meaningful "foam density doesn't double" behaviour and prevents overflow.
