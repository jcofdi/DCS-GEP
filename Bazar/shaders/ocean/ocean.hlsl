...
// [MOD] Issue 3: Corrected refraction IOR from 1.4 to 1.333 (real water).
// This was over-bending refracted rays, making underwater features appear
// more distorted than physically correct.
float refractionFactor = 1.0 / 1.333; // [MOD] was 1.0 / 1.4
...
// [MOD] Issue 7 (partial): Replaced non-physical Fresnel in specular with
// Schlick approximation so the sun specular highlight uses a real Fresnel term.
// Original was F = 1/(1+NdotV) which has no physical derivation.
float3 SpecTerm(const in sInput i) {
	...
	// [MOD] Schlick Fresnel with F0 = 0.02 for water (from IOR 1.333).
	// Original was: float F = 1.0 / (1.0 + NdotV); which is non-physical.
	float F0 = 0.02;
	float F = F0 + (1.0 - F0) * pow(1.0 - VdotH, 5.0); // [MOD]
...
// [MOD] Issue 2: Deep color converted from gamma-space constant to linear.
// Original was used raw in lighting math (gamma-space values in linear pipeline).
// sRGB-to-linear: pow(x, 2.2). Also slightly shifted hue toward physical
// open-ocean blue-green (less cyan, slightly darker).
// Original: { 0.217, 0.46,  0.478 }  (gamma, teal)
// New:      { 0.038, 0.182, 0.197 }  (linear equivalent)
const float3 DeepColor_3 = { 0.038, 0.182, 0.197 }; // [MOD] linearized
...
// [MOD] Issue 4: Corrected R0 from 0.0204 to 0.02 (exact value for IOR 1.333).
// Difference is small but aligns with the enlight path's corrected IOR.
...
float3 calcWaterColor(float3 wPos, float waterDepth, float3 N, float2 reflectionUV, inout float NdotL, float shadow, float3 omni_spot) {
	...
	fi.R0 = 0.02; // [MOD] was 0.0204 — exact F0 for IOR 1.333
	fi.power = 5.0;

	// [MOD] Issue 4: Reduced Fresnel floor from 0.15 to 0.02 (physical F0).
	// A floor of 0.15 made the ocean 15% reflective even when looking straight
	// down, hiding water transparency. Real water at normal incidence is only ~2%.
	float F = clamp(FresnelTerm(fi), 0.02, 1.0); // [MOD] was 0.15
