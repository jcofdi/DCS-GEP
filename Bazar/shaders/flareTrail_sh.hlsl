...
float4  PS_shaderName(PS_INPUT_PARTICLE i, uniform bool bAtmosphere) : SV_TARGET0
{
	...
	// [MOD] Issue F3: Exponential glow decay replacing cubic.
	// Stock used a linear ramp with cubic shaping that confined visible
	// glow to the youngest ~12.5% of the trail with an abrupt cutoff.
	// Real MTV combustion glow decays exponentially from the source --
	// very bright at the pellet, falling off smoothly with distance.
	// exp(-combustionAge * 12) reaches ~0.01 by 38% of trail length,
	// giving a compact but soft-edged hot zone.
	float combustionAge = (1.0 - nAgeInv); // 0 at head, 1 at tail
	float glowFactor = exp(-combustionAge * 12.0);
	glowFactor *= saturate(nAgeInv * 50.0); // fade-in to avoid pop at birth
	glowFactor += params2.z * 20.0 * saturate(1 - (1 - nAgeInv) * 200); // birth flash

	// [MOD] Issue F5: Night perceptual boost on trail self-illumination.
	// The burning pellet has constant luminance, but perceived brightness
	// of the glow within the smoke column is higher at night due to dark
	// adaptation and lower background masking. Uses sun elevation as the
	// signal, matching the approach in flareGlow.fx (Issue F1) and
	// afterburner.fx getGlowModulator().
	// The 0.6 boost (vs 0.9 for the billboard glow) is intentional --
	// trail emission is partially self-absorbed by the smoke column,
	// reducing the night/day contrast compared to the point-source glow.
	float nightGlow = 1.0 + 0.6 * saturate(-gSunDir.y);
	finalColor.rgb += diffuseColor.rgb * diffuseColor.rgb * (glowFactor * 0.5 * nightGlow);

	// [MOD] Issue F2: Thermal color gradient along the trail.
	// Real MTV flares show bright yellow-white at the combustion head
	// (~2000-2300 K blackbody), transitioning through amber/orange to
	// neutral grey smoke. (1.0, 0.85, 0.55) approximates ~2100 K
	// chromaticity in sRGB. The ramp uses pow(x,3) to keep the warm
	// zone compact near the head, matching the short active combustion
	// length of a real flare pellet. Applied after lighting but before
	// atmosphere so that inscatter blends naturally.
	float thermalFactor = pow(saturate(nAgeInv), 3); // 1 at head, 0 at tail
	float3 thermalTint = lerp(float3(1.0, 1.0, 1.0), float3(1.0, 0.85, 0.55), thermalFactor);
	finalColor.rgb *= thermalTint;
...
float4  PS_FLIR_shaderName(PS_INPUT_PARTICLE i, uniform bool bAtmosphere) : SV_TARGET0
{
	...
	// [MOD] Issue F6: Extended IR glow for FLIR path.
	// In the 3-5 um MWIR band, thermal emission from cooling combustion
	// products persists much longer than visible glow. Wien's displacement
	// law puts the Planck peak at ~1000 K around 2.9 um, so even
	// substantially cooled particles radiate strongly in the FLIR band.
	// Exponential decay at rate 6 (vs 12 in the visual path) gives
	// roughly 2x the glow extent, which is conservative for the
	// MWIR-to-visible thermal persistence ratio.
	float combustionAge = (1.0 - nAgeInv);
	float glowFactor = exp(-combustionAge * 6.0);
	glowFactor *= saturate(nAgeInv * 50.0);
