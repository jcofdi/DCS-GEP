...
//		if (delta > 0) {
		if (delta > 0 && delta2 > lerp(delta, 0, saturate(rayEnd.z*0.0002)) ) {		// 5km far to check delta2 
			...
			for (uint j = 0; j < 5; ++j) {
				float mid = (lo + hi) * 0.5;
				float3 midSp = pp + dp0 * mid;
				float midT = -cross(midSp, pos).z / cross(midSp, dir).z;
				float3 midEnd = pos + dir * midT;
				float2 midUV = getScreenCoord(midEnd);
				float midDepth = getDepth(midUV);
				float midDelta = midEnd.z - midDepth - 0.5;
