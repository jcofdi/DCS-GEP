...
// [MOD] Issue 1: Corrected IOR from 1.2 to 1.333 (physical water).
// [MOD] Removed non-physical (NoV + 0.5) fudge factor from exact Fresnel branch.
// The exact Fresnel equation already produces the correct curve; the extra
// scaling was artificially boosting reflectance at steep angles where water
// should be nearly transparent.
float waterFresnel(float NoV) {
	float ior = 1.333; // [MOD] was 1.2 — real water IOR for visible light
#if 1
...
// [MOD] Issue 5: Energy-conserving waterShading.
...
float3 waterShading(float3 pos, float4 viewDir, float3 normal, float3 color, float3 reflection_color, float shadow, float foam, float3 sunLight, float riverLerp) {
	...
	// [MOD] Gate transmitted (refracted + scattered) color by (1 - fresnel) for
	// energy conservation. The reflected fraction is fresnel; the transmitted
	// fraction is (1 - fresnel). Original code blended without this weighting.
	float reflFactor = fresnel * fresnelMult * lerp(1, shadow, max(gSurfaceNdotL, 0) * 0.5);
	color = color * (1.0 - reflFactor) + reflection_color * reflFactor;
...
// [MOD] Issue 6: Removed hardcoded 4.5x brightness multiplier.
// The ambient cube water face should represent actual water luminance.
// The original inflated value caused objects near water to receive unrealistically
// strong upward blue bounce light. Now derived from the same deep color * sunLight
// with a modest 1.5x boost to account for skylight contribution not otherwise
// captured in this simplified sample.
