...
//	return float4(0.5, 0.5, 1, 1);
	...
	// Suppress bilinear bleed film: snap near-flat normals to exactly flat.
	// Threshold 0.01 preserves wet trails (0.010+) and all droplet signals
	// while killing accumulated smear artifacts (0.001-0.005) between drops.
	static const float filmThreshold = 0.01;
	c = lerp(float3(HALF, HALF, 1), c, smoothstep(0.0, filmThreshold, length(c.xy - HALF)));
