...
local function getVoiceChatDevices(dev_type)
	if not mod_sound.getVoiceChatDevices then return {} end
...
viewsCockpit('avionicsMFDEveryFrame')		:setValue(false)	:checkbox():enabledSim()
...
viewsCockpit('mirrorsEveryFrame')			:setValue(false)	:checkbox():enabledSim()	
viewsCockpit('mirrorsSequentialRendering')	:setValue(false)	:checkbox():enabledSim()															
viewsCockpit('mirrorsResolution')			:setValue(1)		:combo({Name('256')	:Value(0),
																		Name('512')	:Value(1),
																		Name('1024'):Value(3),
																}):low(0):medium(1):high(3):VR(1)
...
misc('mgrs_grid_visible')			:setValue(true)		:checkbox()
misc('geo_grid_visible')			:setValue(false)	:checkbox()
